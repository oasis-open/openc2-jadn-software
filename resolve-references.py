...
def resolve(schema: str = 'device-slpf-v1.1.jadn', reference_dir: str = SCHEMA_DIR, output_dir: str = OUTPUT_DIR) -> None:
    print(f'Installed JADN version: {jadn.__version__}\n')
    print(f'{reference_dir}/{schema} -> {output_dir}')
    os.makedirs(output_dir, exist_ok=True)
    filename, ext = os.path.splitext(schema)
    sc = jadn.load_any(os.path.join(reference_dir, schema))                 # Load base package
